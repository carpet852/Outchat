/******************************************************************************
    Filename: CC1120_spi.h
    
    Description: header file that defines a minimum set of neccessary functions
                 to communicate with CC112X over SPI as well as the regisster
                 mapping. 
*******************************************************************************/
#ifndef CC1120_SPI_H
#define CC1120_SPI_H
...
// functions copied from project cc1120_easy_link_vchip_boosterpack_for_MSP430_launchpad/hal_msp_exp430g2_spi.c
...
// functions copied from project cc1120_easy_link_vchip_boosterpack_for_MSP430_launchpad/cc112x_spi.c
uint8 cc112xSpiReadReg(uint16 addr, uint8 *data, uint8 len);
uint8 cc112xSpiWriteReg(uint16 addr, uint8 *data, uint8 len);
uint8 cc112xSpiWriteTxFifo(uint8 *pWriteData, uint8 len);
uint8 cc112xSpiReadRxFifo(uint8 *pReadData, uint8 len);
 ...
 * MACROS
 */
//#define     SPI_BEGIN()           st( CS_N_PORT_OUT &= ~CS_N_PIN; )//NOP();		// st() => see hal_defs.h
#define     SPI_BEGIN()           CS_N_PORT_OUT &= ~CS_N_PIN;
//#define     SPI_TX(x)             st( while (!(UCB0IFG & UCTXIFG)); UCB0TXBUF = x; ) // wait for USCI_B0 TX buffer empty (UCTXIFG bit in UCB0IFG reg)
#define     SPI_TX(x)             while (!(UCB0IFG & UCTXIFG)); UCB0TXBUF = x;  // wait for USCI_B0 TX buffer empty (UCTXIFG bit in UCB0IFG reg)
//#define     SPI_WAIT_DONE()       st( while (UCB0STAT & UCBUSY); )	// wait if bus is busy (UCBBUSY bit in UCB0STAT reg)
#define     SPI_WAIT_DONE()       while (UCB0STAT & UCBUSY);	// wait if bus is busy (UCBBUSY bit in UCB0STAT reg)
#define     SPI_RX()              UCB0RXBUF;
//#define     SPI_END()             st(  CS_N_PORT_OUT |= CS_N_PIN; )//NOP();
#define     SPI_END()             CS_N_PORT_OUT |= CS_N_PIN;
